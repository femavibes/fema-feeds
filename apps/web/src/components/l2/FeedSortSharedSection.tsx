  ...
  return (
    <section className="feed-sort-shared-section">
      ...
      <fieldset className="feed-sorting-tuning-fields feed-sort-shared-fields" disabled={disabled}>
        <div className="feed-sort-shared-dropdown-row">
          <label className="l2-inspector-field">
            ...
            {tuning.decayMode === 'halflife' && (
              <span className="card-hint">Gentle. Score halves every N hours since publish.</span>
            )}
            {tuning.decayMode === 'exponential' && (
              <span className="card-hint">Aggressive. Score compounds down every N hours since publish.</span>
            )}
            {tuning.decayMode === 'rate' && (
              <span className="card-hint">Divides by post_created_hours + 1 — rewards fast engagement.</span>
            )}
          </label>
          <label className="l2-inspector-field">
            Author fairness
        ...
        <label className="l2-inspector-field feed-sort-shared-solo-field">
          Editor score boost
          <input
            type="number"
            min="0"
